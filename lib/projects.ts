    ...
    sections: [
      ...
      },
      {
        ...
        bullets: [
          ...
          "런타임 상태를 영속 모델(MessagePack)로 직렬화해 네트워크 계층과 세이브 계층을 일원화 (직렬화 구현 자체는 별도 매니저에 있어 발췌 범위 밖)",
      ...
      },
      {
        ...
        bullets: [
          ...
          "적 탐지·접근·공격·플레이어 복귀·체력 회복으로 트리 구성. 공격은 선딜·시전·후딜 세 단계로 나누어 각 구간을 따로 다루고, NavMesh 목적지는 매 틱이 아니라 일정 간격으로 갱신",
      ...
      },
      {
        ...
        bullets: [
          ...
          "NetworkManagerEx 한 클래스가 트랜스포트 구성과 서버 시작·종료, 종료 핸드셰이크, 브로드캐스트 등록, 강제 퇴장까지 맡고 있어서 354줄이 되었습니다. 지금이라면 종료 절차만 따로 떼어 별도 클래스로 옮기고, NetworkManagerEx는 연결 상태만 관리하도록 나누겠습니다.",
          "직접 만든 행동트리에서는 블랙보드가 문자열을 키로 쓰고 있어서, 키 이름을 잘못 적어도 컴파일할 때 걸리지 않습니다. 지금이라면 키를 타입이 붙은 형태로 만들어 컴파일러가 잡아 주도록 하겠습니다. 이 밖에 발췌본을 읽으며 확인한 것들은 samples 폴더의 README에 코드 위치와 함께 정리해 두었습니다.",
        ],
      },
    ],
  },
