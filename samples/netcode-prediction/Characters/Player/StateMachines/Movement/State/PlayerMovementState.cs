...
namespace GenshinImpactMovementSystem
{
    public class PlayerMovementState : IState
    {
        ...
        {
            stateMachine.ReusableData.MovementInput = Managers.Input.MovementInput.Value;
        }
        ...
        private float AddCameraRotationToAngle(float angle)
        {
            ...
            {
                Debug.LogWarning("MainCameraTransform is null in AddCameraRotationToAngle");
                return angle;
            }
        ...
        {
            Managers.Input.WalkToggleInput.started += OnWalkToggleStarted;

            Managers.Input.YawInput.started += OnMouseMovementStarted;

            Managers.Input.MovementInput.performed += OnMovementPerformed;

            Managers.Input.MovementInput.canceled += OnMovementCanceled;
        }
        ...
        protected void RotateTowardsTargetRotatoin()
        {
            ...
            stateMachine.ReusableData.DampedTargetRotationPassedTime.y += (float)InstanceFinder.TimeManager.TickDelta;
        ...
        protected void UpdateCameraRecenteringState(Vector2 movementInput)
        {
            ...
            // Add null check for MainCameraTransform
            if(stateMachine.Player.MainCameraTransform == null)
            {
                Debug.LogWarning("MainCameraTransform is null in UpdateCameraRecenteringState");
                return;
            }
        ...
        {
            if(stateMachine.Player.CameraUtility == null)
            {
                Debug.LogWarning("CameraUtility is null in EnableCameraRecentering");
                return;
            }
        ...
        protected void DisableCameraRecentering()
        {
            if(stateMachine.Player.CameraUtility == null)
            {
                Debug.LogWarning("CameraUtility is null in DisableCameraRecentering");
                return;
            }
        ...
        protected virtual void OnWalkToggleStarted(bool pressed)
        {
            stateMachine.ReusableData.ShouldWalk = !stateMachine.ReusableData.ShouldWalk;
        }

        protected virtual void OnMovementCanceled(Vector2 movementInput)
        {
            DisableCameraRecentering();
        }

        private void OnMouseMovementStarted(float yawInput)
        {
            UpdateCameraRecenteringState(stateMachine.ReusableData.MovementInput);
        }

        private void OnMovementPerformed(Vector2 movementInput)
        {
            UpdateCameraRecenteringState(movementInput);
        }
        #endregion
    }
}